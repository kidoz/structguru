...
- Removed `setup_structlog()`. Use `configure()` for runtime setup, the supported
  import-time environment variables for container defaults, and stdlib
  `logging.getLogger(name).setLevel(...)` for third-party logger suppression.
